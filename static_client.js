...
if (APP_CONFIG.mode === 'static') {
  ...
  dataView = function() {
    return note('Shared website: one full update daily, with official patch checks every three hours and an extra collection after a live patch change. Check updates loads the latest publication. It does not start a scrape. Written recommendations need a separate reviewed update. Your picks stay in this browser.') + oldDataView();
  };
  chrome = function() {
    originalChrome();
    ...
    $('#freshness').textContent += ' Daily update target: ' + nextDaily() + ' (your time). Patch checks every three hours; schedules can be delayed.';
