...
## Previous release instructions

# 2.28.2 evidence announcements: installation and rollback
...
Rollback: quit the local app, then run **Roll Back 2.28.2.bat**; it verifies the backup's checksums before restoring 2.28.1 and does not touch `data\`, `snapshots\` or `settings.json`. The website is rolled back by reverting the 2.28.2 merge commit on `main`. No saved data changes format in 2.28.2.
...
Rollback: quit the local app, then run **Roll Back 2.28.1.bat**; it verifies the backup's checksums before restoring 2.28.0 and does not touch `data\`, `snapshots\` or `settings.json`. The website is rolled back by reverting the 2.28.1 merge commit on `main`. No saved data changes format in 2.28.1.
...
Rollback: quit the local app, then run **Roll Back 2.28.0.bat**; it verifies the backup's checksums before restoring 2.27.0 and does not touch `data\`, `snapshots\` or `settings.json`. The website is rolled back by reverting the 2.28.0 merge commit on `main`. Saved offline data does not change format in 2.28.0: after an update or a rollback every saved rank still opens offline (checked with real service workers against a staged 2.27.0 build).
...
Rollback: quit the local app, then run **Roll Back 2.27.0.bat**; it verifies the backup's checksums before restoring 2.26.2 and does not touch `data\`, `snapshots\` or `settings.json`. The website is rolled back by reverting the 2.27.0 merge commit on `main`. Ranks that 2.27.0 saved for offline use are saved as compact cores, which 2.26.2 does not read: after a website rollback each rank is saved again the next time it is opened online, and the 2.27.0 files stay in the browser's storage until its site data is cleared.
