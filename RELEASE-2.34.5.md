...
- **Review of Hotfix 1.17.1.** The re-fetched 1.17 article differs from the reviewed copy only by the hotfix section. Each fix was checked against the reviewed text:
  - **Sanguine Banquet (Valmont):** incorrect mana regeneration was fixed. The plans quote the official launch values (2 mana per nearby minion death, 30 per hero death), which the fix restores.
