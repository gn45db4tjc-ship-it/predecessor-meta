...
function heroTile(p,{favorite=false}={}){const perf=E.displayPerformance(p),review=E.metaReview(p.slug,p.role),plan=E.buildReview(p.slug,p.role),ready=!!plan?.active,tierValue=perf&&!perf.inspection_only?(S.bracket==='gold'&&review?review.tier:perf.tier):null;return `<article class="mobile-hero-card"><div class="meta-hero-identity">${heroButton(p.slug,p.role).replace('</button>',(ready?'':`<span class="chip tag warning no-build-chip">${plan?'Build needs review':'No reviewed build'}</span>`)+'</button>')}</div><div class="mobile-stat">${perf?`${tierValue?tier(tierValue):''} <strong>${pct(perf.wr)}</strong><small>${games(perf.played)}${perf.played<100?' · small sample':''}${perf.inspection_only?' · previous dataset':''}</small>`:`<small>${esc(noStatsText(p.role,p.slug))}</small>`}</div>${favorite?`<button class="favorite-button" data-favorite="${p.slug}|${p.role}" aria-label="Remove ${esc(name(p.slug))} from favorites">★</button>`:''}</article>`;}
...
function guidedHome(){
 ...
 `<section id="mobile-role-list">${compact&&unusual?`<details class="meta-context" data-keep="meta-context"><summary>${esc(overview)}</summary><div class="detail-content">${mobileStatusHTML()}<p>${esc(lead)}</p>${listStatus}</div></details>`:''}${compact?`<h2 class="sr-only">${esc(labels[role])} heroes · ${rows.length} of ${everyone.all.length}</h2>`:`<div class="section-title"><h2>All ${esc(labels[role].toLowerCase())} heroes</h2><small>Showing ${rows.length} of ${everyone.all.length}</small></div>`}${compact?'':listStatus}<div class="mobile-card-list" id="mobile-all-list">${rows.map(r=>heroTile({slug:r.slug,role})).join('')||empty('No hero matches this role and search.')}</div></section>`+
