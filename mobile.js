...
function heroTile(p,{favorite=false}={}){const perf=E.displayPerformance(p),review=E.metaReview(p.slug,p.role);return `<article class="mobile-hero-card"><div class="meta-hero-identity">${heroButton(p.slug,p.role).replace('</button>','<span class="meta-card-cue" aria-hidden="true">Build & match advice <span>↗</span></span></button>')}</div><div class="mobile-stat">${perf?`${perf.inspection_only?'':S.bracket==='gold'&&review?tier(review.tier):tier(perf.tier)} <strong>${pct(perf.wr)}</strong><small>${games(perf.played)}${perf.played<100?' · small sample':''}</small>`:`<small>${esc(noStatsText(p.role,p.slug))}</small>`}</div>${perf?.inspection_only?'<small class="mobile-row-source">Statz '+esc(perf.patch)+' · previous dataset</small>':''}${favorite?`<button class="favorite-button" data-favorite="${p.slug}|${p.role}" aria-label="Remove ${esc(name(p.slug))} from favorites">★</button>`:''}</article>`;}
...
function savedHeroShortcuts(favorites,recent){
 ...
 return `<details class="saved-heroes" data-keep="saved-heroes"><summary>Your heroes · ${favorites.length} favorites · ${recentOnly.length} recent</summary>${favorites.length?`<section id="mobile-favorites"><h2>Favorites</h2><div class="saved-hero-list">${favorites.map(p=>`<div>${shortcut(p)}<button class="favorite-button" data-favorite="${esc(p.slug+'|'+p.role)}" aria-label="Remove ${esc(name(p.slug))} ${esc(labels[p.role])} from favorites">★</button></div>`).join('')}</div></section>`:''}${recentOnly.length?`<section id="mobile-recent"><h2>Recent</h2><div class="saved-hero-list">${recentOnly.map(p=>`<div>${shortcut(p)}</div>`).join('')}</div></section>`:''}</details>`;
}
function guidedHome(){
