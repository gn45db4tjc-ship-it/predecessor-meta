...
function heroTile(p,{favorite=false}={}){const perf=E.displayPerformance(p),review=E.metaReview(p.slug,p.role);return `<article class="mobile-hero-card">${heroButton(p.slug,p.role)}<div class="mobile-stat">${perf?`${perf.inspection_only?'':S.bracket==='gold'&&review?tier(review.tier):tier(perf.tier)} <strong>${pct(perf.wr)}</strong><small>${games(perf.played)}${perf.played<100?' · small sample':''}</small>`:`<small>${esc(noStatsText(p.role,p.slug))}</small>`}</div>${perf?.inspection_only?'<small class="mobile-row-source">Statz '+esc(perf.patch)+' · previous dataset</small>':''}${favorite?`<button class="favorite-button" data-favorite="${p.slug}|${p.role}" aria-label="Remove ${esc(name(p.slug))} from favorites">★</button>`:''}</article>`;}
