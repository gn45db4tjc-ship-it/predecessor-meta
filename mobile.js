...
function guidedHome(){
 ...
 `<section id="mobile-role-list">${compact&&unusual?`<details class="meta-context" data-keep="meta-context"><summary>${esc(overview)}</summary><div class="detail-content">${mobileStatusHTML()}<p>${esc(lead)}</p>${listStatus}</div></details>`:''}${compact?`<h2 class="sr-only">${esc(labels[role])} heroes · ${rows.length} of ${everyone.all.length}</h2>`:`<div class="section-title"><h2>All ${esc(labels[role].toLowerCase())} heroes</h2><small>Showing ${rows.length} of ${everyone.all.length}</small></div>`}${compact?'':listStatus}${buildNote}<div class="mobile-card-list" id="mobile-all-list">${rows.map(r=>heroTile({slug:r.slug,role},{flagBuild:flagBuilds})).join('')||empty('No hero matches this role and search.')}</div></section>`+
