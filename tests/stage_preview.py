...
The committed gold seed was collected on live patch 1.16.4. By default it is staged with the dated review
packet that matches it (tests/fixtures/reviewed-guidance-2.33.0.json.gz, the 14 Sep 1.16.4 review as
released in 2.33.0), so its historical probes keep a coherent reviewed state after the current packet moved
to 1.17. Explicit seeds use the current reviewed_guidance.json. --packet overrides either choice.
"""
import argparse
import gzip
import json
...
HISTORICAL_PACKET = ROOT / 'tests' / 'fixtures' / 'reviewed-guidance-2.33.0.json.gz'


def stage(output, state_dir, seeds=(), packet=None):
    output, state_dir = Path(output).resolve(), Path(state_dir).resolve()
    if packet is None:
        packet = 'current' if seeds else 'historical'
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        override = None
        if packet != 'current':
            source = HISTORICAL_PACKET if packet == 'historical' else Path(packet)
            raw = source.read_bytes()
            override = Path(tmp) / 'reviewed_guidance.json'
            override.write_bytes(gzip.decompress(raw) if raw[:2] == b'\x1f\x8b' else raw)
        previous = publication.base.GUIDANCE_PACKET_OVERRIDE
        publication.base.GUIDANCE_PACKET_OVERRIDE = override
        try:
            manifest = publication.run(state_dir, output, preview_seeds=plain)
        finally:
            publication.base.GUIDANCE_PACKET_OVERRIDE = previous
...
if __name__ == '__main__':
    ...
    parser.add_argument('--packet', default=None, help="'current', 'historical' or a packet path (default: historical for the committed seed, current otherwise)")
    args = parser.parse_args()
    print(json.dumps(stage(args.output, args.state_dir, args.seed, args.packet)))
