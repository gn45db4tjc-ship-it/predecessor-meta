...
(async()=>{
 ...
 for(const width of [320,390,1440])for(const theme of ['dark','light']){
  ...
  const chart=await page.locator('[data-skill-guide] .skill-chart').first().evaluate(t=>({levels:[...t.tHead.rows[0].cells].slice(1).map(c=>c.textContent.trim()),rows:[...t.tBodies[0].rows].map(r=>r.dataset.skillRow),perLevel:[...t.tHead.rows[0].cells].slice(1).map((c,i)=>[...t.tBodies[0].rows].filter(r=>r.cells[i+1].classList.contains('is-ticked')).length),perRow:[...t.tBodies[0].rows].map(r=>r.querySelectorAll('td.is-ticked').length)}));assert.deepEqual(chart.levels,Array.from({length:18},(_,i)=>String(i+1)));assert.deepEqual(chart.rows,['Primary','Secondary','Alternate','Ultimate']);assert(chart.perLevel.every(n=>n===1),'one ticked box per level');assert.deepEqual(chart.perRow,[5,5,5,3]);assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true,'the chart scrolls inside its own box, not the page');await page.locator('[data-skill-level-select]').first().selectOption('6');assert.match(await page.locator('.skill-answer').first().textContent(),/Level 6/);assert.equal(await page.locator('[data-skill-guide] .skill-chart-level[aria-current=step]').first().textContent(),'6');assert.equal(await page.locator('[data-skill-guide]').first().locator('td.is-ticked.is-current').count(),1,'the chosen level has exactly one ticked box');
  ...
  await page.evaluate(()=>{delete companionPrefs.selectedBuilds['khaimera|jungle'];S.locks=[{slug:'khaimera',role:'jungle'}];S.me='khaimera';S.liveVariant=0;S.enemies=[];S.bans=[];save();changeRoute('live');});assert.match(await page.locator('#main').innerText(),/previous app needs to be chosen again/);
  ...
  if(width<700){await page.locator('#main [data-hero="khaimera"]').click();await page.locator('[data-reset-playstyle]').click();}else await page.locator('[data-live-default]').click();
