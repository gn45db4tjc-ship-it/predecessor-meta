...
- Python: 131 tests run, 129 pass, 1 recorded expected failure (defect D), 1 skipped where no local bundle store exists. 2.23.0 had 97.
- JavaScript: 101 tests, all pass; 2 of them assert that defect F still reproduces. 2.23.0 had 78.
- Audit browser suite against the staged build: 23 of 23 verdicts match the ledger. Fixed and enforced: A1–A12, B1–B3, C1–C5, H1. Reproduced and recorded open: E1, G1. The new guards A10–A12, C1 and C5 each fail on the code before the review fixes.
...
- Clean-room source package (80 files, unpacked into an empty folder and tested there): 131 Python tests run, of which 4 are skipped because they need the public seed or the local bundle store, which are not part of the package, and 1 is the recorded expected failure; 101 JavaScript tests pass.
