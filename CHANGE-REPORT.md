...
# Version 2.30.3 — explicit phone interface updates

Fixed the gap where an installed app could keep its old interface while its statistics refreshed. The manifest and HTML now expose the same app release. Foreground and publication checks offer a visible, user-controlled update; More and Sources expose version and manual checking. A fresh document and saved active choices are verified before navigation. Offline, failed saves and mismatched/captive-portal documents keep the current screen usable. Root-document checks use the worker's network-first path. No source statistics, dates, strategy guidance or ranking formulas changed.
